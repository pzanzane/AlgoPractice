...
class RomanToInt {
    ...
    fun romanToInt(s: String): Int {
        ...
        val addNumbers = mutableListOf<Int>()
        ...
        s.forEachIndexed { index, ch ->
            ...
            if (index > 0 && prevInt < number) {
                calculatedNumber = number - addNumbers[runningIndex - 1]
                runningIndex --
                addNumbers.removeAt(runningIndex)
            }

            addNumbers.add(runningIndex, calculatedNumber)
            prevInt = number
            runningIndex ++
        }
